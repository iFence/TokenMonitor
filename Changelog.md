# Changelog

本文件维护每个版本对外发布的更新说明。

编写约定：
- 每个版本使用 `## v版本号` 作为标题，例如 `## v0.2.1`
- 标题下方直接填写会展示在应用更新弹窗中的 Markdown 内容
- 使用 `---` 或下一个版本标题结束当前版本说明

## v0.3.6

### 🚀 优化改进
- **设置页适配窄窗口**：左侧设置菜单改为随窗口一起收窄，优先把宽度留给设置表单；设置项固定为「标签在左、控件在右」的一行布局，窄窗口下不再换行。
- **每日用量热力图方格更大**：去掉热力图左侧的「一 / 三 / 五」星期标签，省下的 30px 全部让给网格，主面板收窄时方格能展示得更大。
- **首页 Cache Hit 环更小更细**：应用卡片上的环状图直径由 48px 缩到 32px、环宽由 10.8px 减到 4.5px，卡片更清爽。
- **TUI 更新说明可滚动**：终端版「更新检查」面板的更新说明超出屏幕时，可用 ↑↓ / PgUp / PgDn / Home / End 滚动查看完整内容。
- **移除主题色设置**：设置页不再提供强调色下拉，界面沿用原有强调色（默认海洋蓝），少一项需要维护的选项。

---

## v0.3.5

### ✨ 新增功能
- **开机自启**：设置页新增「开机自启」开关，登录时自动启动 TokenMonitor（Windows 写注册表 `Run` 项，Linux 写 XDG autostart，macOS 写 LaunchAgent plist）。

### 🚀 优化改进
- **无边框窗口与自定义顶栏**：移除原生标题栏，改为应用内顶栏：左上角显示应用图标与「TokenMonitor」名称，右上角新增关闭按钮（点击后隐藏到系统托盘）；窗口最小宽度收窄至约 557px。
- **顶栏更新时间只显示时分**：上次扫描完成时间格式由 `HH:MM:SS` 改为 `HH:MM`。
- **仪表盘时间范围标签撑满宽度**：今日/昨日/本周/上周/本月/本年六个时间范围按面板宽度均匀分布。
- **项目页按 Token 用量排序**：项目列表按总 Token 数从多到少倒序显示，并移除副标题。
- **Antigravity 图标改为跟随主题**：应用卡片的 Antigravity 图标改为单色 `currentColor`，在明暗主题下保持清晰。

---

## v0.3.4

### ✨ 新增功能
- **Trae 用量统计**：Trae 本地用量数据库加密、无法直接读取，改为解析 tokscale 同步缓存 `%APPDATA%\tokscale\trae-cache\sessions\*.json`（macOS/Linux 为 `~/.config/tokscale/...`，可用 `TOKSCALE_CONFIG_DIR` 覆盖）中的会话级 Token 用量，纳入输入/输出/缓存读写统计；模型名归一化为 tiktoken 风格 id 以便下游定价。需先执行 `tokscale trae login` + `tokscale trae sync`，否则无数据（不报错）。
- **CodeBuddy IDE / 桌面端用量统计**：解析 IDE 扩展存储的会话索引（`%LOCALAPPDATA%\CodeBuddyExtension\Data\...\index.json`）中每个请求的 `usage`，从 `codebuddy-sessions.vscdb` 还原工作区路径，并按请求 id 关联模型，补齐此前仅统计 CLI 会话的空缺。

### 🚀 优化改进
- **CodeBuddy 稳定去重键**：去重键由行号改为 provider 稳定的请求 id，历史行会在下次扫描时自动重建，避免升级后重复统计。
- **仪表盘应用卡片图标**：每张应用卡片名称旁新增 16px 品牌 logo（内置 SVG），随主题前景色染色，保证明暗主题下清晰可见。

---

## v0.3.3

### ✨ 新增功能
- **Gemini CLI 用量统计**：解析 `$GEMINI_CLI_HOME/tmp/*/chats/*.json`（回退 `~/.gemini/tmp/*/chats/*.json`）会话中的 `usageMetadata`，纳入输入/输出/缓存读统计。
- **Qoder 用量统计**：改为读取 Qoder CN 本地 SQLite（`%APPDATA%\QoderCN\SharedClientCache\cache\db\local.db`，macOS/Linux 对应路径，可用 `TOKEN_MONITOR_QODER_CN_DB_PATH` 覆盖），纳入 Token 用量。
- **WorkBuddy SQLite 兜底**：当会话 jsonl 未记录用量（如额度耗尽）时，改从 `workbuddy.db` 的 `session_usage` 读取会话级输入 Token，补齐此前漏统计。

### 🚀 优化改进
- **OpenCode 多数据源**：除 `opencode.db` 外同时读取 `opencode-stable.db`（其它渠道）与旧版 `storage/message/*.json`，并按消息 id 跨源去重，避免重复统计。
- **WSL 多根目录**：CodeBuddy、WorkBuddy 改为自动发现并纳入各 WSL 发行版的数据，与本地数据按根目录命名去重。
- **OpenClaw 旧版数据**：在 `.openclaw` 与 `.clawdbot` 之外，追加 `.moltbot`、`.moldbot` 旧版会话目录。
- **DeepSeek 未压缩会话**：兼容未压缩的 `session.jsonl`（此前仅支持 `.jsonl.zstd`）。
- **数据源对照文档**：新增 `docs/data-sources.md`，逐工具列明本应用与参考项目的读取路径及去重约定。

---

## v0.3.2

### 🐛 修复
- **Linux 发布包改为静态 musl 构建**：官方 `linux-x64` tar.gz 包不再动态链接 glibc，可在 glibc ≥ 2.17 的所有发行版（含 RHEL 8 系 / Anolis 8 / AlmaLinux 8 的 glibc 2.28）直接运行，修复旧包在这些系统上报 `GLIBC_2.29 / GLIBC_2.39 not found` 无法启动的问题。
- **Linux 更新检查可用**：修复 `tokenmonitor-tui -u/--check-update` 与交互面板在 Linux 上始终报 "no matching update asset for platform" 的问题，现在会匹配 `-linux-x64` 的 tar.gz 资产（优先静态 musl 包）。

### 🚀 优化改进
- **TUI 当日分时跟随热力图选中日期**：总览页逐小时柱状图与「当日分时」面板现展示热力图中选中日期的逐小时统计，不再固定显示今日；选中今日时仍会高亮当前小时。

---

## v0.3.1

### ✨ 新增功能
- **Linux 支持（TUI）**：终端版现可运行于 Linux，数据目录按 XDG 规范落到 `~/.local/share/tokenmonitor`，便携布局检测、文件管理器打开等平台能力已补齐。
- **Linux TUI 发布包**：GitHub Release 新增 `linux-x64` tar.gz 便携包，`tokenmonitor-tui` 可直接解压使用。

### 🚀 优化改进
- **TUI 随 Windows 便携包发布**：Windows 便携 zip 同时包含桌面版与 TUI，一次下载双前端可用。
- **TUI 在线更新可用**：TUI 打包进便携包后，更新检查（`--check-update` / 交互面板）可正确识别便携布局，下载对应平台的更新包。

---

## v0.3.0

### ✨ 新增功能
- **终端版（TUI）**：新增 `tokenmonitor-tui` 无界面终端前端，适用于服务器 / 无显示器环境。复用同一 Collector 与 SQLite 存储，Tab 切换三个面板：汇总（热力图 + 按应用/模型明细 + 时间范围切换）、今日小时视图、更新检查；以 `--no-default-features --features tui` 构建，不依赖 GPUI。
- **命令行参数**：两个二进制均支持 `-h/--help` 与 `-V/--version`；TUI 额外支持 `-u/--check-update` 一次性检查更新并退出（退出码：0 无更新 · 2 有更新 · 1 出错），便于脚本集成。
- **TUI 更新检查**：终端版内置 GitHub Releases 更新检查与下载（`u` 检查、`d` 下载、`s` 跳过），启动时静默检查。
- **可配置主题强调色**：设置页新增强调色下拉（海洋 / 碧玉 / 琥珀 / 玫瑰），应用到仪表盘高亮与图表配色。

### 🚀 优化改进
- **项目改名**：rToken 更名为 TokenMonitor，数据库自动从 `%APPDATA%\rToken\rtoken.db` 迁移到 `%APPDATA%\TokenMonitor\tokenmonitor.db`，历史用量完整保留。
- **页面合并**：365 天日报表并入仪表盘页，共享同一滚动；移除独立报告页入口与「应用选择」设置页（所有应用始终追踪）。

---

## v0.2.6

### ✨ 新增功能
- **日报页面**：新增「报告」页，展示过去 365 天的 Token 用量总结统计与贡献式热力图，支持悬停查看单日用量的详细信息。

### 🚀 优化改进
- 修复 GPUI 颜色色相归一化范围（0..=1）问题，图表与热力图配色更准确。

---

## v0.2.4

### ✨ 新增功能
- **支持 Antigravity 用量统计**：读取 `~/.gemini/antigravity-cli/` 与 `~/.gemini/antigravity-ide/` 下各会话 SQLite 库中模型生成步骤的 Token 用量（含输入/输出/缓存读），同时覆盖 Antigravity CLI 与 IDE；Windows 下自动发现并纳入各 WSL 发行版中的会话，与本地数据去重隔离。
- **支持 WorkBuddy 用量统计**：读取 `~/.workbuddy/projects/**/*.jsonl` 会话记录中的 `message.usage` / `providerData.usage`，兼容新旧字段命名，纳入 Token 用量与成本统计。

---

## v0.2.3

### ✨ 新增功能
- **支持 Pi 用量统计**：读取 `~/.pi/agent/sessions/**/*.jsonl` 会话记录中的 assistant 消息、工具内 LLM 调用与压缩/分支摘要用量；Windows 下自动发现并纳入各 WSL 发行版中的 Pi 会话，与本地会话去重隔离。

---

## v0.2.2

### ✨ 新增功能
- **可配置扫描间隔**：设置中可选择 30 秒到 1 小时的自动扫描间隔，修改后立即生效。
- **增量文件扫描**：按文件修改时间与大小跳过未变化的数据源，降低定时重扫的 IO 与内存开销。
- **便携版自动更新**：便携版构建也可检查并应用 GitHub Releases 更新。
- **Windows 深色标题栏**：窗口标题栏跟随深色主题。
- **应用图标与开始菜单快捷方式**：安装包附带应用图标，并注册开始菜单入口。

### 🐛 修复
- 修复图表页顶部刻度超出绘图区域的问题。
- 修复修改扫描间隔后未立即生效、需等待旧周期结束的问题。

### 🚀 优化改进
- 勾选/取消勾选或排序应用时，仅在启用新应用时才触发扫描，其余情况仅刷新视图，避免冗余扫描。

---

## v0.2.1

### ✨ 新增功能
- **OpenCode 适配器**：解析 OpenCode 的会话数据，纳入 Token 用量与成本统计。
- **OpenClaw 适配器**：解析 OpenClaw 的会话数据，纳入 Token 用量与成本统计。
- **DeepSeek Harness 适配器**：解析 `~/.dsh` 下 zstd 压缩的会话数据，纳入用量与成本统计。
- **图表时间范围改为下拉选择**：图表页的时间范围从标签页切换为下拉菜单，更紧凑直观。

---

## v0.2.0

### ✨ 新增功能
- **首个正式发布**：聚合展示 Claude Code、Codex、Gemini CLI、CodeBuddy、OpenCode、Qoder 等 AI 编程工具的 Token 用量与成本。
- **多维度聚合**：按应用、模型、项目、日期聚合展示，支持日/周/月/年时间窗口。
- **图表页**：按天查看用量与成本趋势，支持应用/模型筛选与指标切换。
- **成本计算**：内置 300+ 模型定价，按输入/输出/缓存读写归因成本。
- **应用选择设置**：可启用/禁用并排序要追踪的应用。
- **WSL 合并统计**：自动发现并合并 WSL 发行版中的用量数据。
- **自动更新检查**：启动时检查 GitHub Releases 是否有新版本，在「关于」面板提示。

### 🚀 优化改进
- 大幅降低每 30 秒定时重扫的内存与 IO 开销（指纹跳过无变化数据源 + 聚合下沉 SQL）。

---
