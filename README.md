...
TokenMonitor 的桌面版与 TUI 版都是**单文件、自包含**的本地应用：不内嵌 Electron / Chromium 运行时，安装包仅含可执行文件与必要资源，因此安装体积与常驻内存都明显优于 Electron 类方案。

| 项目 | TokenMonitor | 典型 Electron 应用 |
| --- | --- | --- |
| Windows 安装包（MSI） | ~8.3 MB | 80–150 MB（含 Chromium / Node 运行时） |
| Windows 便携版（免安装 zip，含桌面版 + TUI） | ~10.6 MB | — |
| Linux TUI 静态包（musl，可跑在 RHEL / CentOS 7 系） | ~2.6 MB | — |
| 桌面版常驻内存 | ~115 MB | 150–300+ MB（主进程 + 渲染 / GPU 多进程） |
| TUI 版常驻内存（服务器 / 无显示器） | ~18 MB | — |
| 依赖运行时 | 无，单二进制 | 必须捆绑完整 Chromium + Node |

- **安装包小约 10 倍**：8–10 MB 对比 Electron 常见应用的 80–150 MB，下载、安装与磁盘占用都更低。
- **不捆绑运行时**：单个自包含二进制，没有几百 MB 的 Chromium / Node 运行时，也没有 Electron「一个应用多个进程」的内存放大。
- **内存可控**：桌面版用 GPUI GPU 加速，典型常驻 ~115 MB；TUI 版仅 ~18 MB，适合服务器 / 无显示器环境。
- **内存可回收**：使用 mimalloc，扫描 / 刷新释放的内存会还给系统，RSS 回落，而不是停留在分配器高水位。
- **更安静**：单进程、无常驻后台更新进程；自动更新按需触发。

> 注：内存为本地实测值（桌面版为常规进程名的典型值），实际会随 GPU / 驱动与数据量略有浮动；Electron 为常见应用的典型区间。
